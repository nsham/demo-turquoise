(function (document, $) { // (1)
    ...
    $(document).on('dialog-ready', function (e) { // (2)
       ...
       if (typeof $dialog.attr('action') != "undefined") {
           componentPath  =  $dialog.attr('action').replace('_jcr_content', 'jcr:content');
       } else{
           componentPath = $(".coral-Form--vertical.cq-dialog.foundation-form.foundation-layout-form").attr('action').replace('_jcr_content', 'jcr:content');
