  ...
  CUI.Multifield = new Class(/** @lends CUI.Multifield# */{
    ...
    construct: function (options) {
      ...
      if (this.ol.length === 0) {
        this.ol = $(listTemplate).prependTo(this.$element);
          ...
          if($(this.ol.closest('.foundation-field-edit').find('.coral-Form-fieldlabel')).html() == "Vanity URL") {
				this.ol = seperateCountryOL(this.ol);
          }
      }
    ...
    _addListeners: function () {
      ...
      this.$element.on("click", ".js-coral-Multifield-add", function (e) {
          var limit = $(self.ol).parent().attr('data-item-max-limit') || 5;
    	  if($(self.ol).hasClass("limit-multifield")&&$(".js-coral-Multifield-input").length>=limit){
    		  alert('Cannot add more than ' + limit + ' items');
    	  }else{
            var item = $(fieldTemplate);
			if($(self.ol.closest('.foundation-field-edit').find('.coral-Form-fieldlabel')).html() == "Vanity URL") {
            	item.find(".js-coral-Multifield-placeholder").replaceWith("<div class='coral-vanityDiv' >" + identifyCountry(window.location.href) + "/</div> " + self.script.html().trim());
            } else {
      ...
      this.$element.on("click", ".js-coral-Multifield-remove", function (e) {
          var limit = $(self.ol).parent().attr('data-item-min-limit') || 2;
    	  if($(self.ol).hasClass("limit-multifield")&&($(".js-coral-Multifield-input").length-1<limit)){
    		  alert("Cannot delete, there must be a minimum of " + limit + " items");
    	  }else{
...
//                                console.log(e.message);
  ...
  if (CUI.options.dataAPI) {
    $(document).on("cui-contentloaded.data-api", function (e) {
      CUI.Colorpicker.init($("[data-init~=aiacolorpicker]", e.target));
    });
  }
}(window.jQuery));