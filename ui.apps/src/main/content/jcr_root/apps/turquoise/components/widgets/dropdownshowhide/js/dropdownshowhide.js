...
(function(document, $) {
    "use strict";
//    $(document).off("foundation-contentloaded");
//    $(document).off("selected", ".cq-dialog-dropdown-showhide");

    // when dialog gets injected
    $(document).on("foundation-contentloaded", function(e) {
        //var resourceType = $("[name='./sling:resourceType']").val();
        //if(resourceType == "usgb/components/content/form-checkbox") {
   ...
   function showHide(el){
       ...
       el.each(function(i, element) {
           ...
           if (widget) {
               ...
               if($(element).hasClass("showhide-by-index")) {
                   $(element).parent().parent().find(target).not(".hide").addClass("hide");
    
                   // unhide the target element that contains the selected value as data-showhidetargetvalue attribute
                   $(element).parent().parent().find(target).filter("[data-showhidetargetvalue='" + value + "']").removeClass("hide");
               }
               else {
